...
**Deletion wishes are never platform truth.** The platform cannot delete modules — a "delete module X" turn in the instructions did NOT happen: the entity is still in `app_metadata.json` and its records still exist. Never treat such an entity as gone. You MAY de-prioritize it on the overview (no KPI card, no quick action), but its generated page stays reachable and you never remove its data access.
WRONG: skipping `X` entirely because an instruction turn says "delete X"
RIGHT: `X` gets no overview card, everything else about it stays untouched
