...
export class LivingAppsService {
  ...
  static async getReparaturauftraege(): Promise<Reparaturauftraege[]> {
    ...
    return hydrateRecords(records, 'reparaturauftraege');
  }
  static async getReparaturauftraegeEntry(id: string): Promise<Reparaturauftraege | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Reparaturauftraege;
    return hydrateRecords([record], 'reparaturauftraege')[0];
  }
  static async createReparaturauftraegeEntry(fields: CreateReparaturauftraege): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.REPARATURAUFTRAEGE}/records`, { fields: cleanFieldsForApi(fields as any, 'reparaturauftraege') });
    return { ...data, record_id: data.id };
  }
  static async updateReparaturauftraegeEntry(id: string, fields: Partial<CreateReparaturauftraege>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.REPARATURAUFTRAEGE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'reparaturauftraege') });
    return { ...data, record_id: data.id };
  }
