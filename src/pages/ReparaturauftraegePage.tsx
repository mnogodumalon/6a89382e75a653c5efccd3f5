...
export default function ReparaturauftraegePage() {
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} ${t('in_system', { entity: appLabel('reparaturauftraege') })}`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder={t('search_entity', { entity: appLabel('reparaturauftraege') })}
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('fahrrad_beschreibung')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('reparaturauftraege', 'fahrrad_beschreibung')}
                  {sortKey === 'fahrrad_beschreibung' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('problembeschreibung')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('reparaturauftraege', 'problembeschreibung')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('abgabedatum')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('reparaturauftraege', 'abgabedatum')}
                  {sortKey === 'abgabedatum' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('status')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('reparaturauftraege', 'status')}
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/reparaturauftraege/${record.record_id}`); }}>
                ...
                <TableCell className="text-muted-foreground">{formatDate(record.fields.abgabedatum)}</TableCell>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{lookupLabel('reparaturauftraege', 'status', record.fields.status?.key) ?? record.fields.status?.label ?? '—'}</span></TableCell>
                <TableCell>
      ...
      <ConfirmDialog
        ...
        title={t('delete_entity', { entity: appLabel('reparaturauftraege') })}
