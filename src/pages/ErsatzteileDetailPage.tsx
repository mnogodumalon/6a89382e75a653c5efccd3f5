...
import { formEnhancements } from '@/config/form-enhancements/Ersatzteile';
import { evalComputed } from '@/config/form-enhancements/types';
import { t, appLabel, fieldLabel, localeTag, CURRENCY } from '@/i18n';

export default function ErsatzteileDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<Ersatzteile | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getErsatzteile();
  ...
  async function handleUpdate(fields: Ersatzteile['fields']) {
    if (!record) return;
    await LivingAppsService.updateErsatzteileEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteErsatzteileEntry(record.record_id);
    setDeleteOpen(false);
