...
export default function ErsatzteilePage() {
  ...
  const [records, setRecords] = useState<Ersatzteile[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Ersatzteile | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Ersatzteile | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getErsatzteile());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Ersatzteile['fields']) {
    await LivingAppsService.createErsatzteileEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Ersatzteile['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateErsatzteileEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteErsatzteileEntry(deleteTarget.record_id);
