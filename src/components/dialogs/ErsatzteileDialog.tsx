...
export type ErsatzteileDialogDefaults = Ersatzteile['fields'];

interface ErsatzteileDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Ersatzteile['fields']) => Promise<void>;
...
export function ErsatzteileDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: ErsatzteileDialogProps) {
  const [fields, setFields] = useState<Partial<Ersatzteile['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['bezeichnung', 'lagerbestand', 'preis'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Ersatzteile['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'ersatzteile');
      await onSubmit(clean as Ersatzteile['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Ersatzteile['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'lagerbestand': (
      <div key="lagerbestand" className="space-y-1.5">
        ...
        />
        {showErrors && !fields.lagerbestand && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.ERSATZTEILE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
