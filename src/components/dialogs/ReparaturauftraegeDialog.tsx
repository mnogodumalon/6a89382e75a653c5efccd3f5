...
export type ReparaturauftraegeDialogDefaults = Omit<Reparaturauftraege['fields'], 'status'> & {
...
export function ReparaturauftraegeDialog({ open, onClose, onSubmit, defaultValues, recordId, kundenList, enablePhotoScan = true, enablePhotoLocation = true }: ReparaturauftraegeDialogProps) {
  ...
  const REQUIRED_FIELDS = ['kunde', 'fahrrad_beschreibung', 'problembeschreibung', 'abgabedatum', 'status'] as const;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'fahrrad_beschreibung': (
      <div key="fahrrad_beschreibung" className="space-y-1.5">
        ...
        <Input
          ...
          required
        />
        {showErrors && !fields.fahrrad_beschreibung && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    ...
    'abgabedatum': (
      <div key="abgabedatum" className="space-y-1.5">
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('reparaturauftraege', 'status', 'angenommen') ?? 'Angenommen'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('reparaturauftraege', 'status', 'in_arbeit') ?? 'In Arbeit'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('reparaturauftraege', 'status', 'fertig') ?? 'Fertig'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('reparaturauftraege', 'status', 'abgeholt') ?? 'Abgeholt'}
          </button>
        </div>
