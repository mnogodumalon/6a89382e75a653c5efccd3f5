/**
 * LeihraederDialog — pre-generated create/edit dialog for Leihraeder.
 *
 ...
 * LeihraederDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Leihraeder['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<LeihraederDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Leihraeder';
...
export type LeihraederDialogDefaults = Omit<Leihraeder['fields'], 'groesse'> & {
    groesse?: LookupValue | string;
  };

interface LeihraederDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Leihraeder['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: LeihraederDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  groesse: LOOKUP_OPTIONS['leihraeder']?.['groesse'] ?? [],
};
const NORMALIZE_APPLOOKUPS: Record<string, string> = {
  verliehen_an: APP_IDS.KUNDEN,
};
...
export function LeihraederDialog({ open, onClose, onSubmit, defaultValues, recordId, kundenList, enablePhotoScan = true, enablePhotoLocation = true }: LeihraederDialogProps) {
  const [fields, setFields] = useState<Partial<Leihraeder['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['rahmennummer'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Leihraeder['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'leihraeder');
      await onSubmit(clean as Leihraeder['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["verliehen_an"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const verliehen_anName = raw['verliehen_an'] as string | null;
        if (verliehen_anName) {
          const verliehen_anMatch = kundenList.find(r => matchName(verliehen_anName!, [[r.fields.vorname ?? '', r.fields.nachname ?? ''].filter(Boolean).join(' ')]));
          if (verliehen_anMatch) merged['verliehen_an'] = createRecordUrl(APP_IDS.KUNDEN, verliehen_anMatch.record_id);
        }
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('leihraeder') })
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'rahmennummer': (
      <div key="rahmennummer" className="space-y-1.5">
        ...
        <Input
          ...
          required
        />
        {showErrors && !fields.rahmennummer && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'groesse': (
      <div key="groesse" className="space-y-1.5">
        <Label htmlFor="groesse">{fieldLabel('leihraeder', 'groesse')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            ...
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.groesse) === 'm'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('leihraeder', 'groesse', 'm') ?? 'M'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.groesse) === 'l'}
            onClick={() => setFields(f => ({ ...f, groesse: (lookupKey(f.groesse) === 'l' ? undefined : 'l') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.groesse) === 'l'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'tagespreis': (
      <div key="tagespreis" className="space-y-1.5">
        <Label htmlFor="tagespreis">{fieldLabel('leihraeder', 'tagespreis')}</Label>
        <Input
          id="tagespreis"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'tagespreis')}
          placeholder=""
          value={fields.tagespreis !== undefined ? fields.tagespreis : (computedValues['tagespreis'] ?? '')}
          onChange={e => setFields(f => ({ ...f, tagespreis: clampNumberValue(formEnhancements, 'tagespreis', e.target.value) }))}
        />
      </div>
    ),
    'verliehen_an': (
      <div key="verliehen_an" className="space-y-1.5">
        <Label htmlFor="verliehen_an">{fieldLabel('leihraeder', 'verliehen_an')}</Label>
        <Combobox
          id="verliehen_an"
          placeholder=""
          ...
          value={extractRecordId(fields.verliehen_an)}
          onChange={id => setFields(f => ({ ...f, verliehen_an: id ? createRecordUrl(APP_IDS.KUNDEN, id) : undefined }))}
          onCreateNew={(q) => openCreateKunden("verliehen_an", q)}
  ...
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"verliehen_an": {"vorname": "Vorname", "nachname": "Nachname", "telefonnummer": "Telefonnummer", "email": "E-Mail-Adresse", "stammkunde": "Stammkunde"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.LEIHRAEDER} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
